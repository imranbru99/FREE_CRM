    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-white/5">
            ...
            <tr class="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                ...
                <td class="px-6 py-4 text-slate-500 dark:text-slate-400">{{ $customer->company ?? '—' }}</td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-right">
                    <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <a href="{{ route('admin.customers.edit', $customer) }}" class="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg">
                            <i class="fa-solid fa-pen-to-square"></i>
                        </a>
                        <form action="{{ route('admin.customers.destroy', $customer) }}" method="POST" onsubmit="return confirm('Delete this customer?')">
                            @csrf @method('DELETE')
                            <button class="p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg">
