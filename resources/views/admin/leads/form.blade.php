...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ isset($lead) ? route('admin.leads.update', $lead) : route('admin.leads.store') }}" method="POST">
        ...
        <div class="glass-panel rounded-3xl p-8 shadow-2xl border-white/20 dark:border-white/5 space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" value="{{ old('name', $lead->name ?? '') }}"
                               ...
                               class="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl focus:ring-4 ring-blue-500/10 focus:border-blue-500 outline-none transition-all dark:text-white" required>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-100 dark:border-white/5 pt-8">
                <div class="space-y-2">
                    <label class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 ml-1">Email Address</label>
                    <input type="email" name="email" value="{{ old('email', $lead->email ?? '') }}"
                           placeholder="prospect@company.com"
                           class="w-full px-4 py-3 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl focus:ring-4 ring-blue-500/10 focus:border-blue-500 outline-none transition-all dark:text-white" required>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $lead->phone ?? '') }}"
                           ...
                           class="w-full px-4 py-3 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl focus:ring-4 ring-blue-500/10 focus:border-blue-500 outline-none transition-all dark:text-white">
                </div>
            </div>
            ...
            <div class="space-y-4 border-t border-slate-100 dark:border-white/5 pt-8">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <label class="cursor-pointer group">
                        ...
                        <div class="flex flex-col items-center gap-2 p-4 rounded-2xl border-2 border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 transition-all peer-checked:border-{{ $meta['color'] }}-500 peer-checked:bg-{{ $meta['color'] }}-500/5 group-hover:scale-[1.02]">
